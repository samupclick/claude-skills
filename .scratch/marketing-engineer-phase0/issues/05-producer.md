...
- 2026-09-08 (implementer, T6): two amendments landed in commit 8e55c50 with the gate: the producer now stores the filled template as `1080x1080.html` beside the PNG (the gate reads every rendered word from it; the overlay words were otherwise unchecked), and `--rerender` refuses a fourth version (`attempt 4 is impossible (FR-29)`). The dev batch was re-produced so its assets carry the HTML; creative ids changed.
